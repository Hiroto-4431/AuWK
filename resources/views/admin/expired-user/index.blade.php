<x-app-layout>
  ...
  <div class="border-b border-gray-400">
    <ul class="px-8 flex items-center">
      ...
      <li class="mr-4">
          {{-- <a href="{{ route('admin.expired-user.index') }}">削除済み</a> --}}
                            ...
                            @foreach($expiredUsers as $expiredUser)
                                <tr class="block w-ful flex items-center px-8 py-2 hover:bg-slate-200 border-b">
                                    <td class="block w-4/12 text-left flex items-center">
                                        ...
                                        @endif
                                        <span class="block ml-4">{{ $expiredUser->name }}</span>
                                        @if($expiredUser->authentication === 1)
                                            <img src="{{ asset("images/badge.png") }}" class="ml-2 w-4 object-cover">
                                        @endif
                                    </td>
                                    <td class="block w-3/12 text-left">
                                        @foreach ($universities as $university)
                                            @if ($expiredUser->university_id === $university->id)
                                    ...
                                    <td class="block w-2/12 text-left flex">
                                        <button onclick="location.href='{{ route('admin.user.show', ['user' => $expiredUser->id]) }}'" class="mr-4">
                                            <img src="{{ asset("images/info.png") }}" class="w-6">
                                        </button>
                                        <form id="delete_{{$expiredUser->id}}" method="POST" action="{{ route('admin.expired-user.destroy', ['expiredUser' => $expiredUser->id]) }}" class="mr-4">
                                            @csrf
                                            <a href="#" data-id="{{ $expiredUser->id }}" onclick="deletePost(this)" class="block" class="block">
  ...
  <script>
    function deletePost(e) {
        'use strict';
        if(confirm('完全にデータが削除されますが、本当によろしいですか？')) {
